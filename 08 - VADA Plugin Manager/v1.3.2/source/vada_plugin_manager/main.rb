...
module VADA
  module PluginManager
    VERSION = '1.3.2' unless const_defined?(:VERSION)
    ...
    class << self
      ...
      def fallback_manifest
        {
          'plugins' => [
            ...
            { 'id'=>'mat_cat_goc_nhin','name'=>'Tạo mặt cắt và góc nhìn','latest_version'=>'1.6.0','repo_path'=>'04 - Tạo mặt cắt và góc nhìn','loader_candidates'=>['vada_mat_cat_view.rb'],'reload_candidates'=>['vada_mat_cat_view/main.rb'],'reload_loader'=>false,'extension_names'=>['VADA Tạo Mặt Cắt & View','VADA Tạo mặt cắt và góc nhìn','Tạo mặt cắt và góc nhìn'],'detect_tokens'=>['Tạo Mặt Cắt','Mặt Cắt','Mat Cat','Section View'],'hot_reload'=>true },
      ...
      def match_extension(plugin, extensions)
        ...
        extensions.find do |ext|
          ...
          exact.include?(name) || tokens.any? { |token| !token.empty? && name.include?(token) }
        end
      end
      ...
      def detect_local!(plugin, extensions)
        ...
        if ext || loader
          ...
          version = ext && !ext[:version].empty? ? ext[:version] : detect_version_from_file(loader || ext[:path])
