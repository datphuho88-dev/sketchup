    ...
    class << self
      ...
      def local_plugins
        ...
        manifest.fetch('plugins', []).map do |plugin|
          ...
          p['update_available'] = update_available?(p['local_version'], p['latest_version'])
          p
        end
      end
      ...
      def update_available?(local, remote)
        ...
        return true if local.to_s == '-' || local.to_s == 'đã cài'
      ...
      def fallback_manifest
        {'plugins'=>[
          {'id'=>'tong_dien_tich','name'=>'Tổng diện tích','latest_version'=>'1.0.0','repo_path'=>'VADA-Tong-Dien-Tich','loader_candidates'=>['vada_tong_dien_tich.rb'],'hot_reload'=>true},
          {'id'=>'tong_do_dai_canh','name'=>'Tổng độ dài cạnh','latest_version'=>'1.0.1','repo_path'=>'05 - Tính tổng độ dài cạnh','loader_candidates'=>['vada_tong_do_dai_canh.rb'],'hot_reload'=>true},
          {'id'=>'thong_ke_sat_hop','name'=>'Thống kê sắt hộp','latest_version'=>'1.0.0','repo_path'=>'06 - Thống kê sắt hộp','loader_candidates'=>['vada_thong_ke_sat_hop.rb'],'hot_reload'=>true}
        ]}
      end
      ...
      def check_updates
        begin
          ...
          @last_check = Time.now
          toast('Đã quét trực tiếp GitHub và lấy phiên bản mới nhất', 'ok')
        rescue => e
      ...
      def reload_plugin(id)
        ...
        return toast('Plugin này cần khởi động lại SketchUp sau khi cập nhật', 'warn') unless p['reloadable']
        begin
          load p['loader']
      ...
      def update_plugin(id)
        ...
        begin
          ...
          if p.fetch('hot_reload', true)
            loader = resolve_loader(p)
            load loader if loader
            toast("Đã cập nhật và reload: #{escape_js(p['name'])}", 'ok')
          else
            toast("Đã cập nhật #{escape_js(p['name'])}. Plugin này cần khởi động lại SketchUp.", 'warn')
          end
        rescue => e
      ...
      def update_all
        ...
        msg = "Cập nhật xong: #{success} thành công, #{failed} lỗi"
        msg += ", #{restart_needed} plugin cần restart" if restart_needed > 0
      ...
      def html
        ...
        *{box-sizing:border-box}body{margin:0;background:#000;color:#eee;font-family:Segoe UI,Arial,sans-serif;font-size:13px}
        ...
        .card.has-update{border-color:#603030}.name{font-weight:700;font-size:14px}.meta{color:#999;margin-top:5px}.ok{color:#54d17a}.warn{color:#f0b84d}.new{color:#ff6565;font-weight:700}
        ...
        function renderState(state){const list=state.plugins||[];const root=document.getElementById('list');const n=list.filter(p=>p.update_available).length;document.getElementById('summary').textContent=state.last_check?`Quét GitHub lúc ${state.last_check} · ${n} plugin có bản mới`:'Chưa quét GitHub trong phiên này';root.innerHTML='';list.forEach(p=>{const installed=p.installed?'<span class="ok">● Đã cài</span>':'<span class="warn">● Chưa nhận diện</span>';const version=p.update_available?'<span class="new">Có bản mới</span>':'<span class="ok">Mới nhất</span>';const rbz=p.rbz_url?'Có RBZ':'Chưa có RBZ';root.innerHTML+=`<div class="card ${p.update_available?'has-update':''}"><div><div class="name">${esc(p.name)}</div><div class="meta">Máy: <b>${esc(p.local_version)}</b> · GitHub: <b>${esc(p.latest_version||'-')}</b> · ${installed} · ${version} · ${esc(rbz)}</div></div><div class="btns"><button class="small update ${p.update_available&&p.rbz_url?'':'disabled'}" onclick="sketchup.updatePlugin('${esc(p.id)}')">Cập nhật</button><button class="small ${p.reloadable?'':'disabled'}" onclick="sketchup.reloadPlugin('${esc(p.id)}')">Reload</button></div></div>`})}
